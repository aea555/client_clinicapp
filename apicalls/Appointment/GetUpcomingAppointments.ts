...
export async function GetUpcomingAppointments(userId: number) : Promise<ServiceResult<UpcomingAppointment[], UpcomingAppointment[]>> {
  ...
  const result: ServiceResult<UpcomingAppointment[], UpcomingAppointment[]> = await response.json();
  return result
}